...
int service_init(void)
{
      ...
      print(LOG_INFO,"[bind] IP:Port  %s:%d\n", get_ip(), port);
...
#endif
                        char header[10] = {0};
                        print(LOG_DEBUG,"send pic,size:%d\n",LEN);
                        len = LEN;
...
int service_run(void)
{
      ...
      char buf[1024];
      print(LOG_INFO, "service is running\r\n");
...
#ifdef PRINTF_SIGN
            memset(buf,0,sizeof(buf));
            print(LOG_DEBUG, "accept info: client addr is %s,port %d\n", \
